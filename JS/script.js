...
const Storage = {
    ...
    deleteBoard(boardId) {
        const boards = Storage.getBoards().filter(function (board) {
            return board.id !== boardId;
        });
        const cards = Storage.getCards().filter(function (card) {
    ...
    addCard(boardId, column, title, description, annotation) {
        ...
        const columnCards = cards.filter(function (card) {
            return card.boardId === boardId && card.column === column;
        });
        const newCard = {
            id: generateId(),
            boardId: boardId,
    ...
    getCardsByBoard(boardId) {
        return Storage.getCards().filter(function (card) {
            return card.boardId === boardId;
        });
    },
};
...
const initSettings = () => {
    ...
    const rotateOpen = () => {
        gear.classList.remove('gear--close');
        gear.classList.add('gear--open');
    };

    const rotateClose = () => {
        gear.classList.remove('gear--open');
        gear.classList.add('gear--close');
    };
